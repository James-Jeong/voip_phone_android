...
/**
 * @class public class FastFourierTransform extends FourierTransform
 * @brief FastFourierTransform class
 */
public class FastFourierTransform extends FourierTransform {
    ...
    public FastFourierTransform(int timeSize, float sampleRate) {
        ...
        if ((timeSize & (timeSize - 1)) != 0) {
            throw new IllegalArgumentException("FFT: timeSize must be a power of two. (timeSize=" + timeSize + ")");
        }
    ...
    public float[] inverse(float[] buffer) {
        ...
        // Denoising + Smoothing Control
        //real = applyHighPassFilter(real, 1.0f);
        //real = applyLowPassFilter(real, 1.0f);
        real = smoothenData(real, 9);
    ...
    private float[] smoothenData(float[] data, float smoothingRate) {
        float[] result = new float[data.length];

        float value = data[0];
        for (int i = 1; i < data.length; i++) {
            float currentValue = data[i];
            value += (currentValue - value) / smoothingRate;
            result[i] = value;
        }
    ...
    private float[] applyHighPassFilter(float[] data, float highPass) {
        int dataLen = data.length;
        float[] appliedData = new float[dataLen];
        float[] filterPoints = new float[dataLen];
        ...
        for (int i = 0; i < dataLen; i++) {
            appliedData[i] = data[i] * filterPoints[i];
        }

        return appliedData;
    }

    private float[] applyLowPassFilter(float[] data, float lowPass) {
        int dataLen = data.length;
        float[] appliedData = new float[dataLen];
        float[] filterPoints = new float[dataLen];
    ...
    private void buildReverseTable() {
        ...
        for (int limit = 1, bit = N / 2; limit < N; limit <<= 1, bit >>= 1)
            for (int i = 0; i < limit; i++)
                reverseArray[i + limit] = reverseArray[i] + bit;
    }
    ...
    private void bitReverseComplex() {
        ...
        for (int i = 0; i < real.length; i++) {
            revReal[i] = real[reverseArray[i]];
            revImag[i] = imag[reverseArray[i]];
        }
