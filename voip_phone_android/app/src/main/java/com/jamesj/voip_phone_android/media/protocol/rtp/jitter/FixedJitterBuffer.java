...
package com.jamesj.voip_phone_android.media.protocol.rtp.jitter;

import com.jamesj.voip_phone_android.media.protocol.rtp.RtpPacket;
import com.jamesj.voip_phone_android.media.protocol.rtp.base.Memory;
...
public class FixedJitterBuffer implements JitterBuffer, Serializable {
    ...
    public FixedJitterBuffer (String name, RtpClock clock, int jitterBufferSize) {
        ...
        Logger.d("(%s) FixedJitterBuffer is created.", name);
    }
    ...
    public void write (RtpPacket packet, RtpFormat format) {
        if (format == null) {
            Logger.w("(%s) No format specified. Packet dropped!", name);
            return;
        }
        ...
        try {
            ...
            Logger.w("(%s) Could not acquire write lock for jitter buffer. Dropped packet.", name);
        } finally {
            if (locked) {
                this.lock.unlock();
            }
    ...
    private void safeWrite (RtpPacket rtpPacket, RtpFormat format) {
        ...
        if (rtpPacket.getTimeStamp() < this.arrivalDeadLineTimeStamp) {
            Logger.w("(%s) drop packet: arrivalDeadLineTimeStamp=%s, packet=%s",
        ...
        if (rtpFrame == null) {
            Logger.w("(%s) Fail to allocate the rtp frame. (rtpTotalLength=%s)", name, rtpTotalLength);
            return;
        }
        ...
        if (duration < 0 && queue.size() > 1) {
            Logger.w("(%s) Something messy happened. Resetting jitter buffer!", name);
            reset();
            return;
        }
        ...
        if (queue.size() > QUEUE_SIZE) {
            Logger.w("(%s) Jitter Buffer overflow! (duration=%sms, queueSize=%s)", name, duration, queue.size() - 1);
            dropCount++;
    ...
    public RtpFrame read () {
        ...
        try {
            ...
            Logger.w("(%s) Could not acquire reading lock for jitter buffer.", name);
            this.ready.set(false);
        } finally {
            if (locked) {
                lock.unlock();
    ...
    public void reset () {
        ...
        try {
            ...
            Logger.w("(%s) Could not acquire lock to reset jitter buffer.", name);
        } finally {
            if (locked) {
                lock.unlock();
            }
    ...
    public void reStart() {
        ...
        Logger.d("(%s) Restarted jitter buffer.", name);
    }
