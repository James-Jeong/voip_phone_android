package com.jamesj.voip_phone_android.media.netty.module.base;
    ...
    public MessageSender start (Bootstrap b) {
        ...
        try {
            ...
            channelFuture.addListener(
                    (ChannelFutureListener) future -> Logger.d("Success to connect with remote peer. (ip=%s, port=%s)", ip, port)
            );
            channel = channelFuture.channel();
        } catch (Exception e) {
            //logger.warn("MessageSender.start.Exception. (ip=%s, port=%s)", ip, port, e);
            return null;
        }
